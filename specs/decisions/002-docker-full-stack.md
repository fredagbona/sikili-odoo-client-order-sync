...
The assessment requires a single-command local environment: web, API, app database, Odoo, and Odoo’s database, with `./addons` mounted at `/mnt/extra-addons` (tracked `addons/.gitkeep` when no custom modules are present).
